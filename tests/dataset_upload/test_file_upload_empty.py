...
def test_file_upload_empty_csv(mock_file_uploader, mock_warning, mock_try_parsing_csv, setup_manager):
    """
    Test handling of an empty CSV file upload.
    """
    manager = setup_manager

    mock_uploaded_file = MagicMock()
    mock_uploaded_file.name = "empty_file.csv"
    mock_uploaded_file.size = 1024  
    mock_uploaded_file.getvalue.return_value = b"" 

    mock_file_uploader.return_value = mock_uploaded_file

    mock_try_parsing_csv.return_value = pd.DataFrame()

    manager.dataset_db.dataset_exists.return_value = False

    manager.dataset_upload_page()

    manager.dataset_db.dataset_exists.assert_called_once_with("empty_file.csv", 1)
